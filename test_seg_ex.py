...
for model in models:
    ...
    for m in models:
        ...
        seg_x_sup = m['model'].seg_mpm(y)
        res['gen_'+model['name']+'_seg_'+m['name'] + '_sup'] = calc_err(x, seg_x_sup)

        m['model'].estim_param_EM(y, 100, V=True)
        seg_x_nsup = m['model'].seg_mpm(y)
        res['gen_' + model['name'] + '_seg_' + m['name'] + '_nsup'] = calc_err(x, seg_x_nsup)
